...
namespace camera {
    ...
    //^ ********************************** 相机初始化 ************************************ //
    void HikCamera::Init(bool debug_flag, const std::string &config_file_path, const std::string &intrinsic_para_file_path, std::chrono::_V2::steady_clock::time_point timeStart,bool use_mfs) {
        handle = NULL;
        ...
        if (!use_mfs_config) {
            ...
            std::cout << "\nUsing MFS configuration file!  "<< std::endl;
        }
        ...
        nRet = MV_CC_GetIntValue(handle, "WidthMax", &stWidth);
        if (MV_OK != nRet) {
        ...
        nRet = MV_CC_GetIntValue(handle, "HeightMax", &stHeight);
        if (MV_OK != nRet) {
            printf("Get HeightMax fail! nRet [0x%x]\n", nRet);
        }
        height = stHeight.nCurValue;  

        printf("Setting maximum resolution: %d x %d\n", width, height);
        ...
        // nRet = MV_CC_SetIntValue(handle, "TransferSize", 1024*1024); // 设置传输块大小为1MB
        // if (MV_OK != nRet) {
        //     printf("Set TransferSize fail! nRet [0x%x]\n", nRet);
        // }
        ...
        if (MV_OK != nRet) {
            printf("Set GrabStrategy fail! nRet [0x%x]\n", nRet);
        }
        ...
        if (!use_mfs_config) {
            ...
            if (GammaEnable) {
                std::cout << "GammaEnable_setted: " << std::boolalpha << GammaEnable << std::endl;
            ...
            //std::cout << "SaturationEnable: " << SaturationEnable << std::endl;
            if (SaturationEnable) {
                std::cout << "SaturationEnable_setted: " << SaturationEnable << std::endl;
        ...
        //********** 设置采集像素格式 **********/
        nRet = MV_CC_SetEnumValue(handle, "PixelFormat",PixelType_Gvsp_BayerRG8); // 目前 RGB
        if (MV_OK == nRet) {
    ...
    bool HikCamera::set(CamerProperties type, float value) {
        switch (type) {
            ...
            case CAP_PROP_SATURATION_ENABLE: {
                std::cout<<"SaturationEnable: "<<value<<std::endl;
    ...
    void HikCamera::set() {
        // Initial part, can be placed after OpenDevice
        ...
        if (use_mfs_config) {
            // Load camera properties from MFS file - adjust the path as needed
            // You might want to make this path configurable as well
            std::string mfs_path = CameraConfigFilePath;  // Reuse the config path or add a new member variable
            
            nRet = MV_CC_FeatureLoad(handle, mfs_path.c_str());
            if (MV_OK != nRet) {
                printf("Load Feature fail! nRet [0x%x]\n", nRet);
            } else {
                printf("Successfully imported camera properties from MFS file: %s\n", mfs_path.c_str());
            }
        }
    }
    //^ ********************************** 重置相机配置 ************************************ //
    ...
    bool HikCamera::CamInfoShow() {
        ...
        if (MV_CC_GetBoolValue(handle, "GammaEnable", &bGammaEnable) != MV_OK) {
            printf("Gamma: 不支持\n");
        } else {
            if (bGammaEnable && MV_CC_GetFloatValue(handle, "Gamma", &fValue) == MV_OK) {
                printf("Gamma:%f\n", fValue.fCurValue);
            } else {
                printf("Gamma: 0\n");  // 如果不支持或未启用，显示为0
            }
        ...
        printf("白平衡值:%d\n", stParam.nCurValue);
        // 查看色度
        if (MV_CC_GetIntValue(handle, "Hue", &stParam) != MV_OK) {
            printf("色度: 不支持\n");
        }
        // 查看传输速率
        ...
        printf("传输速率 :%d Mbps\n", stParam.nCurValue);
        // 查看连接选择
        MV_CC_GetIntValue(handle, "DeviceConnectionStatus", &stParam) || nRet;
        printf("连接选择 :%d Mbps\n", stParam.nCurValue);
        // 查看链路速度
    ...
    //^ ********************************** 相机取图 ************************************ //
    bool HikCamera::ReadImg(cv::Mat &image, uint64_t *p_device_timestamp, int64_t *p_host_timestamp) {
        pthread_mutex_lock(&mutex);
        bool hasNewFrame = !frame_empty;
        
        if (!frame_empty) {
            image = std::move(camera::frame);
            if (p_device_timestamp){
                *p_device_timestamp = camera::current_device_timestamp;
    ...
    void *HikCamera::HKWorkThread(void *arg) {
        ...
        // 使用更高效的MV_CC_GetImageBuffer方式
        MV_FRAME_OUT stImageInfo = {0};
        MV_CC_PIXEL_CONVERT_PARAM stConvertParam = {0};
        
        // 预分配转换后图像的内存
        unsigned char *m_pBufForSaveImage = (unsigned char *)malloc(camera::g_nPayloadSize * 3); // 为RGB分配3倍空间
        ...
        while (true) {
            ...
            if (MV_OK == nRet) {
                ...
                camera::current_host_timestamp = host_timestamp;
                frame_empty = 0;
                pthread_mutex_unlock(&mutex);
            ...
            // 释放图像缓存 - 配合MV_CC_GetImageBuffer必须使用
    ...
    //^ ********************************* 相机畸变矫正 ************************************ //
    bool HikCamera::undistProcess(cv::Mat srcImg) {

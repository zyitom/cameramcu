 ...
 *  @return 成功，返回MV_OK；错误，返回错误码
 *  @remarks Windows下，可选择OpenGL渲染模式，支持PixelType_Gvsp_RGB8_Packed，PixelType_Gvsp_BGR8_Packed，PixelType_Gvsp_Mono8三种像素格式图像大小超过4GB的渲染，注意Linux下不支持，其他渲染模式不支持。
 ...
 *  @return Success, return MV_OK. Failure, return error code
 *  @remarks Under Windows, OpenGL rendering mode can be selected, supporting three pixel formats: PixelType_Gvsp_RGB8_Packed，PixelType_Gvsp_BGR8_Packed，and PixelType_Gvsp_Mono8 for rendering images with a size exceeding 4GB. 
             Note that, rendering images with a size exceeding 4GB is not supported under Linux, and other rendering modes are not supported.
 ...
 *  @remarks 调用该接口可以设置SDK内部图像缓存节点个数，在调用MV_CC_StartGrabbing开始抓图前调用。
             ...
             SDK实际分配的节点个数 = SDK内部预分配的个数 + 用户分配的节点(MV_CC_SetImageNodeNum);
             该接口不支持MV_CAMERALINK_DEVICE 类型的设备。
 ...
 *  @remarks 该接口定义了四种取流策略，用户可以根据实际需求进行选择。具体描述如下：
             ...
             UpcomingImage:在调用取流接口时忽略输出缓存列表中所有图像，并等待设备即将生成的一帧图像。
			 该策略支持MV_GIGE_DEVICE、MV_USB_DEVICE设备，注意MV_USB_DEVICE设备不支持 MV_GrabStrategy_UpcomingImage 模式；
			 同时该策略也不支持MV_CAMERALINK_DEVICE、MV_1394_DEVICE、GenTL类型的设备；
...
*  @~chinese
*  @brief   枚举采集卡
*  @param   nTLayerType        [IN]             采集卡接口类型 eg: (MV_GIGE_INTERFACE | MV_CAMERALINK_INTERFACE | MV_CXP_INTERFACE| MV_XOF_INTERFACE）
 ...
 *  @remarks 该接口用于判断设备当前是否处于组播状态，解决客户端枚举时需要打开设备判断组播的问题。
             仅支持标准GigE Vision （MV_GIGE_DEVICE）设备。
 ...
 *  @return 成功，返回MV_OK；错误，返回错误码 
 *  @remarks 该接口支持4G以上超大图的PNG/TIFF存图，其他非超大图像支持BMP/JPEG/TIFF/PNG
			 JPEG格式最大支持宽高为65500
 *  @~english
 *  @brief  Save the image file.
 *  @param  handle                      [IN]            Device handle
...
*  @remarks 可以使用 MV_CC_GetPayloadSize 获取缓存大小，并使用 MV_CC_AllocAlignedBuffer 分配空间，之后进行MV_CC_RegisterBuffer注册
            start前调用,要求调用3次以上，即至少注册3块内存到SDK中； （U口/网口相机会使用部分注册内存节点作为缓存，所以MV_CC_GetValidImageNum 小于实际注册外部内存个数）
